...
import {
  ...
  deleteUserStart,
  deleteUserSuccess,
  signOutSuccess, // ✅ FIXED: Import added
...
const DashboardProfile = () => {
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      const profilePictureUrl = getFileUrl(uploadedFile.$id);
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        window.location.href = "/sign-in"; // redirect on delete
      }
    } catch (error) {
      console.log(error);
  ...
  const handleSignout = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        dispatch(signOutSuccess()); // ✅ FIXED
        window.location.href = "/sign-in"; // optional redirect
      }
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div className="max-w-lg mx-auto p-3 w-full">
      ...
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        ...
        <div
          className="w-32 h-32 self-center cursor-pointer overflow-hidden"
